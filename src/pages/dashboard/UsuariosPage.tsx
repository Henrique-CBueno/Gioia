...
const schemaContrato = z
	.object({
		startContractDate: z.string().min(1, "Data de início obrigatória"),
		endContractDate: z.string().min(1, "Data de fim obrigatória"),
	})
	.refine((data) => data.endContractDate >= data.startContractDate, {
...
type ContratoForm = z.infer<typeof schemaContrato>;
...
function ModalEditarContrato({
	...
	const {
		...
		defaultValues: {
			startContractDate: usuario.startContractDate ?? "",
			endContractDate: usuario.endContractDate ?? "",
		},
	});
	...
	}, [reset, usuario.endContractDate, usuario.startContractDate]);

	const onSubmit = async (data: ContratoForm) => {
		setErro(null);
		try {
			await atualizarDatasContratoUsuario(usuario.id, data);
			onSucesso();
		} catch {
			setErro("Erro ao atualizar as datas do contrato.");
		}
	};
	...
	return (
		<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
			<div className="w-full max-w-md bg-white rounded-2xl shadow-xl border border-gray-100 p-6">
				<div className="flex items-center justify-between mb-4">
					<div>
						<h3 className="font-bold text-[#333333] text-base">Editar datas do contrato</h3>
						<p className="text-xs text-gray-400 mt-0.5">
							{usuario.firstName} {usuario.lastName} &mdash; {usuario.email || usuario.username}
						</p>
					</div>
				...
				<form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-3">
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
						<div className="flex flex-col gap-1">
							...
							<input
								{...register("startContractDate")}
								type="date"
								className={inputClass(!!errors.startContractDate)}
							/>
							{errors.startContractDate && (
								<p className="text-red-500 text-xs">{errors.startContractDate.message}</p>
							)}
						</div>
						<div className="flex flex-col gap-1">
							...
							<input
								...
								type="date"
								className={inputClass(!!errors.endContractDate)}
							/>
							{errors.endContractDate && (
								<p className="text-red-500 text-xs">{errors.endContractDate.message}</p>
							)}
						</div>
					</div>
...
export default function UsuariosPage() {
	...
	const [usuarioContratoSelecionado, setUsuarioContratoSelecionado] = useState<UsuarioDTO | null>(null);
	const [sucessoSenha, setSucessoSenha] = useState<string | null>(null);
	const [sucessoContrato, setSucessoContrato] = useState<string | null>(null);
	...
	const handleSucessoContrato = async (nome: string) => {
		setUsuarioContratoSelecionado(null);
		setSucessoContrato(`Datas de contrato de ${nome} atualizadas com sucesso.`);
		setTimeout(() => setSucessoContrato(null), 5000);
		await carregar();
	};
	...
	return (
		<div className="max-w-5xl mx-auto flex flex-col gap-8">
			...
			{usuarioContratoSelecionado && (
				<ModalEditarContrato
					usuario={usuarioContratoSelecionado}
					onClose={() => setUsuarioContratoSelecionado(null)}
					onSucesso={() =>
						handleSucessoContrato(
							`${usuarioContratoSelecionado.firstName} ${usuarioContratoSelecionado.lastName}`,
						)
					}
				/>
			)}
						...
						<div className="overflow-x-auto">
							...
							<thead>
								<tr className="border-b border-gray-100 bg-[#F4F7F9]">
									...
									<th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Contrato</th>
							...
							<tbody>
								{usuarios.length === 0 && (
									<tr>
										<td colSpan={5} className="text-center text-gray-400 py-8">Nenhum usuário encontrado.</td>
									</tr>
								)}
								{usuarios.map((u) => (
									<tr key={u.id} className="border-b border-gray-50 hover:bg-[#F4F7F9]/60 transition-colors">
										...
										<td className="px-5 py-3.5 text-gray-500">
											<div className="flex flex-col">
												<span>{u.startContractDate || "Não informado"}</span>
												<span>{u.endContractDate || "Não informado"}</span>
											</div>
										</td>
										...
										<td className="px-5 py-3.5 text-right">
											<div className="flex items-center justify-end gap-2">
												<button
													type="button"
													onClick={() => setUsuarioContratoSelecionado(u)}
